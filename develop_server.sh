...
SRV_LOG=$BASEDIR/srv.log
PELICAN_PID=$BASEDIR/pelican.pid
PELICAN_LOG=$BASEDIR/pelican.log
...
function start_up(){
  ...
  shift
  $PELICAN --debug --autoreload -r $INPUTDIR -o $OUTPUTDIR -s $CONFFILE $PELICANOPTS >> $PELICAN_LOG 2>&1 &
  pelican_pid=$!
  echo $pelican_pid > $PELICAN_PID
  cd $OUTPUTDIR
  $PY -m pelican.server >> $SRV_LOG 2>&1 &
  srv_pid=$!
